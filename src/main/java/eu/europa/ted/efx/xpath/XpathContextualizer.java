...
public class XpathContextualizer extends AbstractXpathBaseListener {
	...
	public static String contextualize(final String  contextXpath, final String xpath) {
		...
		final AbstractXpathLexer lexer = new AbstractXpathLexer(CharStreams.fromString(String.format("%s, %s", contextXpath, xpath)));
	...
	public String getContextualizedXpath() {
		...
		while (!this.stack.isEmpty()) {
			...
			Queue<String> pathQueue = this.stack.pop();
			...
			Queue<String> contextQueue = this.stack.isEmpty() ? null : this.stack.pop();
			
			if (contextQueue != null) {

				// First we will "consume" all nodes that are the same in both xPaths.
				while (!contextQueue.isEmpty() && !pathQueue.isEmpty() && Objects.equals(contextQueue.peek(), pathQueue.peek())) {
					contextQueue.poll();
					pathQueue.poll();
				}
				...
				// We start building the resulting relativeXpath by appending any nodes remaining in the pathQueue.
				...
				// For each step remaining in the contextQueue we prepend a back-step (..) in the resulting relativeXpath.
				while (!contextQueue.isEmpty()) {
					if(!contextQueue.poll().startsWith("/@")) { // consume the step
						relativeXpath = "../" + relativeXpath;	// prepend a backstep if the step is not an attribute reference 
					}
				}
			...
			result = relativeXpath + (result.isEmpty() ? "" : ("\n" + result));
